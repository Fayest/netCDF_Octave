...
% III Output
%     monthlydata for specific area or dot(max=min for lon and lat)
%-------------------------------------------------------------------------------
...
llo= length(lon);
lla= length(lat);
h=waitbar(0,'monthly rain processing...');
  for i =sy:ey    %year
    for j=1:12    %month
      if j<=9
        nf = ([pwd,'/data/',name,num2str(i),'0',num2str(j),'.nc']);
        dat = ncread([pwd,'/data/',name,num2str(i),'0',num2str(j),'.nc'],par)*con;
        for k=1:llo    %longitude
          for l=1:lla  %latitude
            monthlydata((i-sy)+1,j,k,l)= nansum(dat(lon(k),lat(l),:));
          end
        end
      elseif j>9
